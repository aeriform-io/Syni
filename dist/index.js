...
// Initialize
var _argv = _yargs2.default.usage([_safe2.default.red('Usage:'), '--type, -t         {jpg,png,sgi,tif,dpx,tga,exr} (default: png)', '--start-id, -i     [num]                         (default: 0)', '--write-with, -c   [num]                         (optional)     (--render)', '--write-delay, -d  [relative time]               (default: 5ms) (--render)', '--buffer-delay, -b [relative time]               (default: 0ms) (--render)', '--prefix, -p       [string]                      (default: frame)', '--write, -w        [num:filesize,...]', '--render, -r                                     (optional)', '--zero-byte, -z                                  (optional)     (--render)', '--shuffle                                        (optional)', '', _safe2.default.underline('Examples.'), 'osk --type png -i 5 --write 5:12kb', 'osk --type dpx --render --write 5:10.2mb,3:10.4mb,8:10mb --shuffle'].map(function (e) {
  return e.replace(/([\{\[].+[\}\]])/, _safe2.default.blue('$1')).replace(/(\(.+\))/, _safe2.default.yellow('$1'));
}).join(_os2.default.EOL)).demand(['write']).implies('buffer-delay', 'render').implies('write-with', 'render').implies('write-delay', 'render').alias('t', 'type').alias('i', 'start-id').alias('c', 'write-with').alias('d', 'write-delay').alias('b', 'buffer-delay').alias('z', 'zero-byte').alias('r', 'render').alias('p', 'prefix').alias('w', 'write').epilog('Copyright (C) ' + new Date().getFullYear()).argv;
